...
research_questions = re.findall(r"^## 研究问题 \d+: (.*)$", content, re.MULTILINE)
print(f"研究问题数量: {len(research_questions)}")
for q in research_questions:
    print(f"  - {q}")

paper_ids_in_matrix = set(re.findall(r"\| (paper_\d+) \|", content))
...
print(f"\n## 综述引用检查")
review_path = Path("synthesis/review_draft.md")
if review_path.exists():
    review_content = review_path.read_text(encoding="utf-8")
    refs_in_review = set(re.findall(r"paper_\d+", review_content))
