...
**首轮结论（对本路线）**：跨本体 / sim-实 对齐迁移已有直接证据（040/041/042，
3 篇）；
...
- CLASS（DTW 动作序列相似度监督的 Soft InfoNCE，把视觉表示拉向动作序列；
  动作序列是监督信号而非编码器输入）[paper_044](https://arxiv.org/abs/2508.01600)——对齐度量的可操作形态。
- ProGAL-VLA（符号-实体对齐度量 + 前瞻推理；语言漠视条件互信息从
  0.36-0.57 降到 0.08-0.19）[paper_045](https://arxiv.org/abs/2604.09824)。

**缺口结论（记录）**：
- RQ4.4（对齐度量本身）半补：无专门"action↔text/vision 对齐度量"论文，
  CLASS/ProGAL 的对比/条件互信息是可借形态，但都非"度量即目标"；
