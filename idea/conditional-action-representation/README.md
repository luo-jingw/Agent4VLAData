...
action 空间上存在表征分布：每个 `(o_t, a_t)` 对对应一个高维表征 r；给定
条件 c 与目标 g，表征场上的条件分布经 match 函数查询契合度并据此采样，
采样结果经本体特定解码器映射回 action。
...
需要结构假设才可学。两者收敛到同一结构：学习对象不是直接监督动作标签，
而是为每个观测-动作对估计高维表征，目标条件下的 match 查询替代标量
价值估计——**action 空间上的表征分布 + 条件目标下的 match 查询**。

## 3 定义（六条）

**基本对象**：`(o_t, a_t)` 对——观测 + 动作，最 naive 形式。不含未来输入，不含目标。

1. **表征**：`r = f(o_t, a_t)`，为每个观测-动作对赋予高维表征，构成 action
   空间上的表征场；表征序列构成轨迹表征，与文本目标同域。f 任务无关
   （g 不进输入）。
2. **分布**：给定条件 c 与目标 g，表征场上存在条件分布 `p(R | g, c)`，
   支撑集 = 可达结果流形。多模态合法，不保证唯一最优。
3. **query**：`m(r, g)` 度量表征与目标的契合度。策略 = 在 `p(R | g, c)` 上
   按 match 采样。match 是查询算子：被查询的对象是高维表征，评分是其在
   目标方向上的投影，不是对象本身；完整结构保留在表征里。
...
**标量场是特例**：表征沿目标方向投影为标量即退化为标量场。本框架的评分
与投影可退化为标量，但高维分布结构保留在表征中。
...
- **训练目标未来感知**：f 与 p 的估计以动作条件的未来表征预测为目标，
  预测在表征空间而非像素空间。理由：未来信息可以经训练目标烘焙进
  表征，无需未来作为输入；VLA-JEPA 的 leakage 警告——未来只作监督、不作输入。
- **未来与可观测性分工**：未来期望由训练目标烘焙进 f；Case B（同 (o,a)
  不同后续）的可观测性缺口由 (g, c) 补齐（最小充分上下文）。完全可观测时
  (o, a) 唯一决定未来分布，无需消歧；部分可观测时同一 (o, a) 对应多个未来，
  区分责任转移到条件 (g, c)（ISSUE-003 的机理）。
...
| 未来信息 | 训练目标烘焙（未来表征预测） | 2026 批判线否定重建目标；未来信息经训练目标进入表征 | 未来进输入（leakage 警告） |
