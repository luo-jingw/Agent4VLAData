...
action 空间上存在表征分布：每个 `(o_t, a_t)` 对携带高维表征 r（而非标量
回报），给定条件 c 与目标 g，经 match 函数查询契合度并据此采样，再经
本体特定解码器映射回 action。类比 Q-learning 的"为每个动作赋预期值"，
此处预期值是高维表征而非标量。
...
需要结构假设才可学。两者收敛到同一结构，其核心类比是 **Q-learning 的高维
推广**：不直接以动作标签的 loss 监督，而是为每个动作赋予一个"预期"——
Q-learning 赋的是标量回报 Q(s,a)，本框架赋的是高维表征 r；目标条件下用
match 函数做查询，等价于价值投影（SR 的 wᵀψ），但对象保留高维结构而非
标量。即：**action 空间上的表征分布 + 条件目标下的 match 查询**。
...
3. **query**：`m(r, g)` 度量表征与目标的契合，扮演价值查询的角色
   （SR 的 wᵀψ 投影），但查询对象保留高维结构而非标量。策略 = 在
   `p(R | g, c)` 上按 match 采样。match 是查询算子，不是定义。
...
表征在目标方向 w 上的投影。本框架与 Q-learning 的关键差异：学习目标不是
直接监督 action 的 loss，而是为每个 action 赋予"预期"——Q-learning 赋标量
回报，本框架赋高维表征；match 查询对应投影运算，但完整的高维分布结构
保留在表征里。
