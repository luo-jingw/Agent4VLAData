...
- 文档与交付语言：中文。
- **文档内容边界**：持久化文档只记录项目本身的事实、设计、接口、约束、
  依据与未决问题，面向没有参与对话的读者，自包含地描述当前结果。
  - 对写作方式、范围、沟通对象、工作节奏的要求直接体现为写作行为，
    不转述为文档内容。
  - 概念或逻辑被修正后，直接更新对应表述，不保留对话痕迹。
  - 括号只用于术语解释、单位、公式条件与引用。
  - 对作者的行为要求遵循即可，不写入正文；对系统的技术约束写成约束描述。
...
- **场景假设（2026-09-06，working assumption，待实验验证）**：当前任务
  ...
  **边界条件（[paper_026](https://arxiv.org/abs/2412.03252)）**：速度无关性只在**无接触动力学**
...
- 现有数据处理方法 = 包内 `reference/g2-orion05-resample-training/`
  （自 zip 解压，只读参照，方法说明在其 `docs/` 下；实现另起文件，
  不修改该包）。
...
- 版本控制：GitHub remote 见 Credentials。`g2-orion05-resample-training.zip`
  ...
  报告"PDF 缺失"，属预期。
- 项目技能：  调研工作流 skill 已复制为项目技能 `.claude/skills/research_skill/`
  （含 subagents 与检查脚本）。
